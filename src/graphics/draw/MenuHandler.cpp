...
void menuHandler::RadioPresetPicker()
{
    ...
    bannerOptions.bannerCallback = [](int selected) -> void {
        if (selected == Back) {
            ...
            config.lora.modem_preset = meshtastic_LoRaConfig_ModemPreset_LONG_SLOW;
        } else if (selected == radiopreset_LongModerate) {
            config.lora.modem_preset = meshtastic_LoRaConfig_ModemPreset_LONG_MODERATE;
        } else if (selected == radiopreset_LongFast) {
            config.lora.modem_preset = meshtastic_LoRaConfig_ModemPreset_LONG_FAST;
        } else if (selected == radiopreset_MediumSlow) {
            config.lora.modem_preset = meshtastic_LoRaConfig_ModemPreset_MEDIUM_SLOW;
            ...
            config.lora.modem_preset = meshtastic_LoRaConfig_ModemPreset_SHORT_SLOW;
        } else if (selected == radiopreset_ShortFast) {
            config.lora.modem_preset = meshtastic_LoRaConfig_ModemPreset_SHORT_FAST;
        } else if (selected == radiopreset_ShortTurbo) {
            config.lora.modem_preset = meshtastic_LoRaConfig_ModemPreset_SHORT_TURBO;
        }
        service->reloadConfig(SEGMENT_CONFIG);
        rebootAtMsec = (millis() + DEFAULT_REBOOT_SECONDS * 1000);
    };
