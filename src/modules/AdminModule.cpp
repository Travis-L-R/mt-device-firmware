...
void AdminModule::handleSetConfig(const meshtastic_Config &c)
{
    ...
    case meshtastic_Config_lora_tag: {
        ...
        } else if (config.lora.fem_lna_mode != meshtastic_LoRaConfig_FEM_LNA_Mode_NOT_PRESENT) {
    ...
    case meshtastic_Config_destinations_tag:
        LOG_INFO("Set config: Destinations");
        config.has_destinations = true;
        config.destinations = c.payload_variant.destinations;
        break;
    }
...
void AdminModule::handleGetConfig(const meshtastic_MeshPacket &req, const uint32_t configType)
{
    ...
    if (req.decoded.want_response) {
        ...
        case meshtastic_AdminMessage_ConfigType_DEVICEUI_CONFIG:
            ...
            break;
        case meshtastic_AdminMessage_ConfigType_DESTINATIONS_CONFIG:
            LOG_INFO("Get config: Destinations");
            res.get_config_response.which_payload_variant = meshtastic_Config_destinations_tag;
            res.get_config_response.payload_variant.destinations = config.destinations;
            break;
        }
